...
func TestBuildEvidenceRebasePlanHandlesV4ItemEvidenceAndRefusesUnsafeClaimRewrite(t *testing.T) {
	const (
		...
		newBase    = "2222222222222222222222222222222222222222"
		head       = "3333333333333333333333333333333333333333"
	)
	oldURI, err := diffuri.Build(diffuri.Reference{Repository: repository, Base: oldBase, Head: head, Kind: "line", Path: "app.go", Side: "new", Start: 7, End: 7})
	if err != nil {
		t.Fatal(err)
	}
	newURI, err := diffuri.Build(diffuri.Reference{Repository: repository, Base: newBase, Head: head, Kind: "line", Path: "app.go", Side: "new", Start: 7, End: 7})
	if err != nil {
		t.Fatal(err)
	}
	document := &saga.Saga{
	...
	changes := gitdiff.ChangeSet{
		Repository: repository, BaseOID: newBase, HeadOID: head,
		Atoms: []gitdiff.Atom{{Key: "line:app.go:new:7", URI: newURI, Kind: "line", Path: "app.go", Side: "new", Line: 7}},
	}

	plan, err := buildEvidenceRebasePlan(document, changes, oldBase, false, time.Now().UTC())
	if err != nil {
		t.Fatal(err)
	}
