...
var commandOrder = []string{
	"init", "add-chapter", "add-section", "add-fragment", "cover",
...
2. "change-saga status --json" lists the uncovered product changes. Its
   "uncovered", "orphans", "overlaps", and "targets" arrays are always present,
   so treat an empty "uncovered" array as the completion signal.
...
4. "change-saga cover" connects a focused fragment or landmark to the exact diff atoms
   ...
   writing them.
5. When attaching many selectors, pipe newline-delimited JSON records to
...
Run "change-saga status --json <name>.saga" as the coverage work queue, and
"change-saga query gaps --kind uncovered" to page through the same gaps while
reading. Attach only the exact diff atoms a fragment or landmark explains, with
