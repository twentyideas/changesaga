...
func cover(ctx context.Context, args []string, out io.Writer, stdin io.Reader) error {
	...
	plan := func(locked *saga.Saga) error {
		var planErr error
		planned, planErr = planCoverage(locked, records, files, now, !*dryRun)
		return planErr
	}
	if *dryRun {
...
// disk.
func planCoverage(document *saga.Saga, records []coverRecord, files []saga.DiffFile, now time.Time, create bool) ([]plannedRecord, error) {
	...
	for i, record := range records {
		...
		if create {
			diffDir, err = store.EnsureDirWithin(document.Root, diffDir)
			if err != nil {
				return nil, recordError(records, i, err)
			}
		}
