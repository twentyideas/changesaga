...
func TestCoverageManifestProvesBothMappingDirections(t *testing.T) {
	...
	// Coverage records that a body exists for this path without carrying it;
	// the rows themselves come from /api/file-diff when the reviewer opens the
	// file. TestFileDiffEndpointServesCoverageAndTargetedBodies proves the
	// endpoint still renders exactly those rows.
	if !handler.HasDiff {
		t.Fatalf("coverage file did not offer an inspectable diff: %#v", handler)
	}
	...
	if len(flow.Files) != 1 || flow.Files[0].Path != "src/api/handler.go" || flow.Files[0].AtomCount != 2 || !flow.Files[0].HasDiff {
