...
#   * on macOS the code signature is verified when `codesign` is present;
...
resolve_latest() { # print the newest release tag
	...
	if [ "$DOWNLOADER" = curl ]; then
		effective="$(curl --fail --silent --location --proto '=https' \
			--proto-redir '=https' --tlsv1.2 --output /dev/null \
			--write-out '%{url_effective}' "$url" || true)"
	else
		effective="$(wget --https-only --quiet --output-document /dev/null \
...
expected="$(awk -v want="$ARCHIVE" '$2 == want || $2 == "*" want {print $1}' \
	"$TMPDIR_SAGA/SHA256SUMS" | head -n 1)"
...
tar -xzf "$TMPDIR_SAGA/$ARCHIVE" -C "$TMPDIR_SAGA"
[ -f "$TMPDIR_SAGA/$BIN_NAME" ] || die "archive did not contain $BIN_NAME"
chmod +x "$TMPDIR_SAGA/$BIN_NAME"

if [ "$OS" = darwin ] && command -v codesign >/dev/null 2>&1; then
	unsigned=""
	if codesign --verify --strict "$TMPDIR_SAGA/$BIN_NAME" >/dev/null 2>&1; then
		...
		authority="$(codesign --display --verbose=2 "$TMPDIR_SAGA/$BIN_NAME" 2>&1 |
	...
	if [ -n "$unsigned" ]; then
		...
		log "         fetched by curl carries no quarantine attribute. Gatekeeper"
		log "         is left exactly as it is; nothing here bypasses it."
	fi
fi

if ! "$TMPDIR_SAGA/$BIN_NAME" version >/dev/null 2>&1; then
	die "the downloaded binary did not run on this machine"
fi
log "verified $("$TMPDIR_SAGA/$BIN_NAME" version)"
...
# atomic and a running `saga` process is never overwritten in place.
